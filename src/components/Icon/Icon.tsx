...
const Icon: React.FC<iconProps> = React.memo(({ code, color, ...other }) => {
  const [active, setActive] = useState(false);
  const icon = icons[code];
  const viewBox = icon ? icon.viewBox : '0 0 1024 1024';
  const pathList = (icon ? icon.paths : paths).map((item: iconType) => (
    <path
      key={item.d}
      d={item.d}
      fill={active ? color || '#ccc' : color || item.fill || '#ccc'}
      onMouseEnter={() => {
        setActive(true);
      }}
      onMouseLeave={() => {
  ...
  return (
    <svg
      viewBox={viewBox}
      css={css`
        width: ${1.2 * FONT_BASE}px;
        height: ${1.2 * FONT_BASE}px;
        font-size: 0;
        line-height: 0;
        margin: 0;
        padding: 0;
